...
if ($authorized) {
    ...
    //get the domain and number settings
    $sql = "select domain_uuid, message_number_uuid from v_message_numbers ";
    $sql .= "where message_number = :message_number ";
    $parameters['message_number'] = $message_to;
    $database = new database;
    $row = $database->select($sql, $parameters, 'row');
    if (is_array($row) && @sizeof($row) != 0 ) {
        $domain_uuid = $row['domain_uuid'];
        $message_number_uuid = $row['message_number_uuid'];
    }
    ...
    if ($num_media > 0) {
        for($i=0; $i < $num_media; $i++) {
            ...
            if ($media_ext !== 'xml') {
                ...
                $array['message_media'][$i]['message_number_uuid'] = $user_uuid;
    ...
    //file_put_contents($_SERVER['DOCUMENT_ROOT']."/app/messages/last_msg/".$user_uuid, strtotime("now"));
/*
