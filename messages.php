...
//get message numbers
	if (if_group("superadmin")) {
		...
		$sql_from = ", v_message_users as u ";
		$sql_where =  "where u.user_uuid = :user_uuid and n.message_number_uuid = u.message_number_uuid and ";
		$parameters['user_uuid'] = $_SESSION["user_uuid"];
	}
	$sql = "select n.message_number_uuid, n.message_number from v_message_numbers as n ";
	$sql .= $sql_from;
	$sql .= $sql_where;
	$sql .= " n.domain_uuid = :domain_uuid ";
	$sql .= "and n.message_number_enabled = 'true' ";
	$sql .= "order by n.message_number asc ";
...
//new message layer
	if (permission_exists('message_add')) {
		...
		echo "				<form id='message_new' method='post' enctype='multipart/form-data' action='sw_send.php'>\n";
		...
		if (is_array($message_numbers) && sizeof($message_numbers) != 0) {
			...
			foreach ($message_numbers as $row) {
				echo "							<option value='".$row['message_number_uuid'].":".$row['message_number']."'>".format_phone($row['message_number'])."</option>\n";
			}
			echo "							</select>\n";
		}
		else {
...
//show the content
	...
	if (permission_exists('message_number_add')) {
		echo "				<a href='message_numbers.php'><input type='button' class='btn' alt=\"".$text['label-message_numbers']."\" value=\"".$text['label-message_numbers']."\"></a>\n";
	...
	echo "	<tr>";
	echo "		<th width='30%'>Message Number</th>";
	echo "	</tr>";
	echo "	<tr>";
	echo "<td class='vtable' align='left'>\n";
	echo "<select id='message_number' name='message_number' onchange='unload_thread();' class='formfld' style='width: auto;'>\n";
	echo "		<option value='' selected='selected' disabled='disabled'>".$text['title-message_numbers']."</option>\n";
	foreach ($message_numbers as $row) {
		echo "	<option value='".$row['message_number_uuid'].":".$row['message_number']."'>".format_phone($row['message_number'])."</option>\n";
	}
	echo "</td>";
	...
	echo "		<th width='30%' style='text-align: left; font-weight: normal; padding-left: 15px;' id='contact_current_name'></th>\n";
	echo "		<th width='10%' style='text-align: center; white-space: nowrap;'><nobr>".$text['label-messages']."<nobr></th>\n";
	echo "		<th width='30%' style='text-align: right; font-weight: normal; padding-right: 15px;' id='message_from'></th>\n";
	echo "	</tr>\n";
	echo "	<tr>\n";
	echo "		<td id='contacts' valign='top'><center>&middot;&middot;&middot;</center></td>\n";
	echo "		<td id='thread' colspan='3' valign='top' style='border-left: 1px solid #c5d1e5; padding: 15px 0 15px 15px;'><center>&middot;&middot;&middot;</center></td>\n";
	echo "	</tr>\n";
	echo "</table>\n";
...
//js to load messages for clicked number
	...
	$refresh_thread = is_numeric($_SESSION['message']['refresh_thread']['numeric']) && $_SESSION['message']['refresh_thread']['numeric'] > 0 ? $_SESSION['message']['refresh_thread']['numeric'] : 10; //default (seconds)
	...
	echo "	function refresh_contacts(message_number) {\n";
	echo "		clearTimeout(timer_contacts);\n";
	echo "		$('#contacts').load('messages_contacts.php?message_number=' + message_number + '&sel=' + $('#contact_current_number').val(), function(){\n";
	echo "			$('div#contact_list').css('max-height', $(window).height() - 470);\n";
	echo "			timer_contacts = setTimeout(refresh_contacts, contacts_refresh, message_number);\n";
	echo "		});\n";
	echo "	}\n";

	echo "	function load_thread(contact_number, contact_uuid, message_number_uuid) {\n";
	echo "		clearTimeout(timer_thread);\n";
	echo "		$('#thread').load('messages_thread.php?contact_number=' + encodeURIComponent(contact_number) + '&contact_uuid=' + encodeURIComponent(contact_uuid) + '&message_number_uuid=' + encodeURIComponent(message_number_uuid), function(){\n";
	echo "			$('#message_from').html($('#message_number option:selected').html());\n";
	...
	echo "				refresh_contacts($('#message_number').val());\n";
	echo "				timer_thread = setTimeout(refresh_thread_start, thread_refresh, contact_number, contact_uuid, message_number_uuid);\n";
	echo "			});\n";
	echo "		});\n";
	echo "	}\n";
	...
	echo "		$('#message_from').html('');\n";
	echo "		$('#contact_current_number').val('');\n";
	echo "		$('#contact_current_name').html('');\n";
	echo "		refresh_contacts($('#message_number').val());\n";
	echo "	}\n";

	echo "	function refresh_thread(contact_number, contact_uuid, message_number_uuid, onsent) {\n";
	echo "		$('#thread_messages').load('messages_thread.php?refresh=true&contact_number=' + encodeURIComponent(contact_number) + '&contact_uuid=' + encodeURIComponent(contact_uuid) + '&message_number_uuid=' + encodeURIComponent(message_number_uuid), function(){\n";
	echo "			$('div#thread_messages').animate({ 'max-height': $(window).height() - 470 }, 200, function() {\n";
	echo "				$('#thread_messages').scrollTop(Number.MAX_SAFE_INTEGER);\n"; //chrome
	echo "				$('span#thread_bottom')[0].scrollIntoView(true);\n"; //others
						//note: the order of the above two lines matters!
	...
	echo "				if (onsent != 'true') {\n";
	echo "					timer_thread = setTimeout(refresh_thread, thread_refresh, contact_number, contact_uuid, message_number_uuid);\n";
	echo "				}\n";
	echo "			});\n";
	echo "		});\n";
	echo "	}\n";
...
//refresh controls
	...
	echo "	function refresh_thread_stop(number, contact_uuid, message_number_uuid) {\n";
	echo "		clearTimeout(timer_thread);\n";
	?>			document.getElementById('thread_refresh_state').innerHTML = "<img src='resources/images/refresh_paused.png' style='width: 16px; height: 16px; border: none; margin-top: 3px; cursor: pointer;' onclick=\"refresh_thread_start('" + number + "', '" + contact_uuid + "' , '" + message_number_uuid + "');\" alt=\"<?php echo $text['label-refresh_enable']; ?>\" title=\"<?php echo $text['label-refresh_enable']; ?>\">";<?php
	echo "	}\n";

	echo "	function refresh_thread_start(number, contact_uuid, message_number_uuid) {\n";
	echo "		if (document.getElementById('thread_refresh_state')) {\n";
	?>				document.getElementById('thread_refresh_state').innerHTML = "<img src='resources/images/refresh_active.gif' style='width: 16px; height: 16px; border: none; margin-top: 3px; cursor: pointer;' onclick=\"refresh_thread_stop('" + number + "', '" + contact_uuid + "' , '" + message_number_uuid + "');\" alt=\"<?php echo $text['label-refresh_pause']; ?>\" title=\"<?php echo $text['label-refresh_pause']; ?>\">";<?php
	echo "			refresh_thread(number, contact_uuid, message_number_uuid);\n";
	echo "		}\n";
	echo "	}\n";
