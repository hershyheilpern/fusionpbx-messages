...
//get number_uuid of messages to load
	$contact_uuid = (is_uuid($_GET['contact_uuid'])) ? $_GET['contact_uuid'] : null;
	$contact_number = $_GET['contact_number'];
	$message_number_uuid = $_GET['message_number_uuid'];
...
//check if user is this message_user of this message_number
	...
	if (if_group("superadmin") || if_group("admin") || is_uuid($message_user)) {
		...
		//mark messages for loaded thread as read
			$sql = "update v_messages set message_read = 'true' ";
			$sql .= "where message_from like :contact_number ";
			$sql .= "and message_number_uuid = :message_number_uuid ";
		...
		//get messages
			...
			$sql .= $sql_from;
			$sql .= $sql_where;
			$sql .= " (message_from like :contact_number ";
			$sql .= "or message_to like :contact_number) ";
			$sql .= "and message_number_uuid = :message_number_uuid ";
			$sql .= "and domain_uuid = :domain_uuid ";
			$sql .= $since;
			...
			$parameters['message_number_uuid'] = $message_number_uuid;
			$parameters['domain_uuid'] = $domain_uuid;
			$parameters['contact_number'] = '%'.ltrim($contact_number, '+1');
			$database = new database;
...
//css styles
	...
	//output messages
		...
		echo "<script>\n";
		//set current contact
			echo "	$('#contact_current_number').val('".$contact_number."');\n";
	...
	if (!$refresh) {
		...
		if (permission_exists('message_add')) {
			...
			echo "		<td align='center'><span id='thread_refresh_state'><img src='resources/images/refresh_active.gif' style='width: 16px; height: 16px; border: none; cursor: pointer;' onclick=\"refresh_thread_stop('".$contact_number."','".$contact_uuid."','".$message_number_uuid."');\" alt=\"".$text['label-refresh_pause']."\" title=\"".$text['label-refresh_pause']."\"></span></td>\n";
		...
		//js to load messages for clicked number
			...
			echo "					refresh_thread('".$contact_number."', '".$contact_uuid."', '".$message_number_uuid."', 'true');\n";
			echo "				}\n";
			echo "		});\n";
			echo "	});\n";
